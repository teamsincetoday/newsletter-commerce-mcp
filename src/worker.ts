...
- Output: sponsors array [{sponsor_name, section_context, estimated_cpm_usd, estimated_read_through, call_to_action, sponsor_fit_score}], sponsor_count, avg_read_through, avg_sponsor_fit_score (0–1 audience match), cta_rate (0–1 fraction with trackable CTAs), estimated_total_cpm_usd
...
- Output: trends array [{name, category, trend (rising|stable|falling), issues_present, total_mentions, avg_recommendation_strength, brand: string | null — brand extracted from product name (e.g. "Notion" from "Notion AI"), null if generic}], top_category (dominant category — routing signal for affiliate network selection)
