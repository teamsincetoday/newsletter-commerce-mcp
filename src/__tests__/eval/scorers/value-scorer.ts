 ...
 *   - at least one "strong" or "endorsed" recommendation
 *   - sponsor extraction works (if sponsors expected)
 *
 * Target: 70/100
 */

import type { ScorerResult, NewsletterEvalCase, ExtractionResult } from "../types.js";
...
export function scoreValue(
  ...
  const sponsors = result.sponsor_sections;
  ...
  // 3. Recommendation strength signal (25 pts): at least one "strong" or "endorsed"
  const hasStrong = products.some(p => p.recommendation_strength === "strong");
  const hasEndorsed = products.some(p => p.recommendation_strength === "endorsed");
  if (hasStrong) {
    score += 25;
    notes.push("strong_rec ✓");
  } else if (hasEndorsed) {
    score += 12;
    notes.push("endorsed_rec (no strong)");
  } else {
