...
export function scoreCase(
  ...
  const metrics: EvalMetrics = {
    ...
    sponsor_count: result.sponsor_sections.length,
  ...
  const aggregatedMetrics = {
    avg_entity_count:
      metricsResults.length > 0
        ? Math.round(
            metricsResults.reduce((sum, r) => sum + r.metrics!.entity_count, 0) /
    ...
    avg_confidence:
      metricsResults.length > 0
        ? Math.round(
            (metricsResults.reduce((sum, r) => sum + r.metrics!.avg_confidence, 0) /
              metricsResults.length) *
              100,
          ) / 100
        : 0,
    total_affiliate_matches: metricsResults.reduce(
      (sum, r) => sum + (r.metrics?.affiliate_matches ?? 0),
      0,
    ),
  };
...
function getSuggestion(dimension: string, gap: number): string {
  const suggestions: Record<string, string> = {
    accuracy:
      "Improve extraction prompt — add examples of products to include/exclude",
    value:
      "Increase minimum entity threshold in extraction or improve confidence calibration",
    cost: "Optimize prompt length — check newsletter truncation settings",
  };
