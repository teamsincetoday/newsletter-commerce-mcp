/**
 * SQLite cache + usage metering for Newsletter Commerce MCP.
 *
 * Two concerns in one file (both lightweight):
 * 1. NewsletterCache — stores extraction results keyed by newsletter_id
...
export class NewsletterCache {
  ...
  constructor(dbPath?: string) {
    ...
    // Guard against ":memory:" before calling resolve() — resolve(":memory:") would
    // produce a bogus absolute path instead of the in-memory sentinel.
    ...
    this.stmtGet = this.db.prepare(
      `SELECT data FROM extraction_cache WHERE newsletter_id = ?`
    );

    this.stmtSet = this.db.prepare(
      `INSERT OR REPLACE INTO extraction_cache (newsletter_id, data, created_at)
       VALUES (?, ?, ?)`
    );
  ...
  get(newsletterId: string): ExtractionResult | null {
    const row = this.stmtGet.get(newsletterId) as { data: string } | undefined;
    if (!row) return null;
    try {
  ...
  set(newsletterId: string, result: ExtractionResult): void {
    const now = Math.floor(Date.now() / 1000);
    this.stmtSet.run(newsletterId, JSON.stringify(result), now);
  }
...
let _instance: NewsletterCache | null = null;

/**
 * Get the shared cache instance (singleton per process).
 * Pass a custom dbPath on first call to override the default location.
 */
export function getCache(dbPath?: string): NewsletterCache {
  if (!_instance) {
    _instance = new NewsletterCache(dbPath);
  }
  return _instance;
}
